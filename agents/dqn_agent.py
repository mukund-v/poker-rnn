'''
PyTorch implementation of DQN agent for rlcard platform
...
'''
deep q learning agent. uses agent wrapper as specified:
...
Parameters:
    ...
    lr (float) : learning rate to use for training learner net
...
class DQNAgent():
    ...
    def __init__(self,
        ...
        self.learner_net = DQNet(state_shape, num_actions, hidden_layers).to(device)
        self.target_net = DQNet(state_shape, num_actions, hidden_layers).to(device)
    ...
    def eval_pick_action(self, state, use_max=True):
        with torch.no_grad():
            ...
            exp_qvals = self.softmax(self.target_net(state_obs))[0].cpu().detach().numpy()
